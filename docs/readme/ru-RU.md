...
Рабочий процесс релиза применяет `config/distribution.toml`, чтобы изменить идентификатор zed-i18n, информацию About и путь к манифесту обновлений. В результате путь автоматического обновления меняется на `zed-i18n`.

## Известные ограничения

Большинство строк пользовательского интерфейса — меню, кнопки, всплывающие подсказки, параметры, описания действий — заменяются напрямую. Однако некоторые имена действий, которые формируются динамически во время выполнения в Палитре команд или Keymap Editor, требуют отдельного патча и пока не охвачены.

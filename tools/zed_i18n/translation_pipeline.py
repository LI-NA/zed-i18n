...
def _batch_prompt(base_prompt: str, batch_payload: dict[str, Any]) -> str:
    return "\n\n".join(
        ...
        for part in [
            ...
            "`entries`. For `settings_enum_variant_label` and "
            "`settings_enum_discriminant_label`, treat the source as a visible settings option "
            "label; use sibling options and any `source_comment` in `context_group` before "
            "choosing a glossary row, and keep the label compact. "
